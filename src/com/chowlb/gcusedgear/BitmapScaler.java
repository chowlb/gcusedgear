...
class BitmapScaler {
    ...
    BitmapScaler(URL url, int newWidth)
            throws IOException {
    	InputStream is = null;
    	is = url.openConnection().getInputStream();
    	Log.e("chowlb", "Url is " + url.toString());
    	try {
            is = url.openConnection().getInputStream();
            Log.e("chowlb", "GOT CONNECTION");
            Size size = getRoughSize(is, newWidth);
            Log.e("chowlb", "Size is " + size);
            try {
                is = url.openConnection().getInputStream();
                Log.e("chowlb", "2nd input stream");
                roughScaleImage(is, size);
                Log.e("chowlb", "Rough Scale image success");
                scaleImage(newWidth);
            } finally {
                is.close();
            }
